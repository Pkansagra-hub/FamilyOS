...
class CognitiveSpan:
    ...
    def set_cognitive_load(self, working_memory_usage: float, attention_score: float) -> None:
    ...
    def set_device_context(self, device_id: str, is_family_device: bool = False) -> None:
    ...
    def record_memory_transformation(self, 
                                   input_size: int, 
                                   output_size: int, 
...
class MemoryTracer:
    ...
    @contextmanager
    def start_cognitive_span(self, 
                           cognitive_stage: str,
                           memory_operation: str,
                           envelope: MemoryEnvelope) -> ContextManager[CognitiveSpan]:
        ...
        with self.tracer.start_as_current_span(span_name) as span:
            ...
            cognitive_span.span.set_attribute("envelope.cognitive_trace_id", envelope.cognitive_trace_id)
            cognitive_span.span.set_attribute("envelope.api_plane", envelope.api_plane.value if envelope.api_plane else "unknown")
            cognitive_span.span.set_attribute("envelope.user_id", envelope.user_id or "unknown")
            cognitive_span.span.set_attribute("envelope.device_id", envelope.device_id or "unknown")
            
            # Set memory context if available
            if envelope.memory_spaces:
                cognitive_span.set_memory_context(envelope.memory_spaces, envelope.family_context)
    ...
    @contextmanager
    def start_memory_operation_span(self, 
                                  operation: str,
                                  envelope: MemoryEnvelope) -> ContextManager[CognitiveSpan]:
